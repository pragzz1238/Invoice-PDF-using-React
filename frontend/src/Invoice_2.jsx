...
//This code is to get the base64 String in the console.
...
const Invoice2 = () => {
  ...
  useEffect(() => {
    // Fetch the invoice data from your Express backend
    axios.get('http://localhost:8081/api/invoice')
      .then((response) => {
  ...
  // Define a function to render the PDF content
  const MyDocument = () => (
    <Document>
        ...
        {({ blob }) => {
          if (blob) {
            ...
            reader.onloadend = () => {
              ...
              // Send the base64String to the backend using Axios
              // axios.post("http://localhost:8081/api/baseString", { pdfData: base64String })
              //   .then((response) => {
              //     console.log("PDF sent to the backend:", response.data);
              //   })
              //   .catch((error) => {
